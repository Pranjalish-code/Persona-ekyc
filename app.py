...
class LiveActionProcessor:
    def __init__(self, target_fps: float, max_frames: int = 200):
        ...
        # thresholds  (relaxed for typical webcams)
        self.ear_thr = 0.25      # was 0.21 – too strict for many setups
        self.ear_consec = 1      # was 2 – catch faster blinks
        self.mar_thr = 0.35      # was 0.55 – required exaggerated mouth open
        self.mar_consec = 1      # was 2 – catch quicker mouth opens

        # debug: live EAR/MAR values (updated every frame)
        self.last_ear = 0.0
        self.last_mar = 0.0
    ...
    def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
        ...
        if self._mp_ok:
            ...
            if lm_xy is not None:
                ...
                if ear < self.ear_thr:
                    ...
                    if self.eye_closed_run >= self.ear_consec:
                        self.blink_count += 1
                    self.eye_closed_run = 0
                ...
                if mar > self.mar_thr:
                    ...
                    if self.mouth_open_run >= self.mar_consec:
        ...
        # ── debug: show live EAR / MAR + detection status ──
        h_img = img.shape[0]
        mp_label = "MP:OK" if self._mp_ok else "MP:FAIL"
        face_label = "Face:YES" if self.face_detected else "Face:NO"
        ear_color = (0, 255, 255) if self.last_ear < self.ear_thr else (200, 200, 200)
        mar_color = (0, 255, 255) if self.last_mar > self.mar_thr else (200, 200, 200)

        cv2.putText(img, f"{mp_label}  {face_label}", (20, h_img - 100),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 1)
        cv2.putText(img, f"EAR: {self.last_ear:.3f}  (thr<{self.ear_thr})", (20, h_img - 70),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, ear_color, 1)
        cv2.putText(img, f"MAR: {self.last_mar:.3f}  (thr>{self.mar_thr})", (20, h_img - 40),
...
with right:
    ...
    with tab_cam:
        ...
        # Auto-refreshing counters (reruns this fragment every 1 s)
        @st.fragment(run_every=1)
        def _live_counters():
            _vp = webrtc_ctx.video_processor if webrtc_ctx else None
            buffered = len(_vp.frames) if _vp is not None else 0
            blink_live = int(getattr(_vp, "blink_count", 0)) if _vp is not None else 0
            mouth_live = int(getattr(_vp, "mouth_open_count", 0)) if _vp is not None else 0

            st.write(f"Buffered frames: **{buffered}**")
            st.write(f"Realtime blinks: **{blink_live}**")
            st.write(f"Realtime mouth opens: **{mouth_live}**")

            # Debug: live EAR / MAR values
            if _vp is not None:
                mp_ok = getattr(_vp, "_mp_ok", False)
                mp_err = getattr(_vp, "_mp_error", "")
                face_det = getattr(_vp, "face_detected", False)
                ear_val = getattr(_vp, "last_ear", 0.0)
                mar_val = getattr(_vp, "last_mar", 0.0)
                ear_t = getattr(_vp, "ear_thr", 0.25)
                mar_t = getattr(_vp, "mar_thr", 0.35)

                col_a, col_b = st.columns(2)
                with col_a:
                    st.write(f"MediaPipe: **{'✅ OK' if mp_ok else '❌ FAIL'}**")
                    st.write(f"EAR: **{ear_val:.3f}** (thr < {ear_t})")
                with col_b:
                    st.write(f"Face detected: **{'✅ YES' if face_det else '❌ NO'}**")
                    st.write(f"MAR: **{mar_val:.3f}** (thr > {mar_t})")

                if not mp_ok and mp_err:
                    st.error(f"MediaPipe init error: {mp_err}")

            # Challenge status
            if _vp is not None:
                status = "— not started"
                if _vp.challenge_active:
                    status = "⏳ ACTIVE"
                elif _vp.challenge_passed:
                    status = "✅ PASS"
                elif _vp.challenge_failed:
                    status = "❌ FAIL"
